...
class sortOfClevr():
    ...
    def add_state_descriptor(self, objects):
    ...
    def get_descriptor(self, objects):
        descriptor = []
        for itr_obj in objects:
            shape = 0 if itr_obj[2] == 'r' else 1 # rect -> 0, circle -> 1
            # area = (self.shape_size*2)**2 if shape == 0 else 2*np.pi*self.shape_size
            descriptor.append(np.array([itr_obj[0], itr_obj[1][0], itr_obj[1][1], shape]))
        return np.expand_dims(np.array(descriptor), axis=2)
    ...
    def build_dataset(self):
        ...
        self.add_state_descriptor(objects)
        ...
        descriptor = (self.get_descriptor(objects), rel_data, norel_data)
    ...
    def driver(self):
        ...
        for _ in tqdm(range(self.train_size)):
            dataset, descriptor = self.build_dataset()
        ...
        print('Saving descriptor traininer...')
        with open(self.descriptor_trainer_path, 'wb') as fd:
            pickle.dump((train_descriptor, test_descriptor), fd)
        print(f'Descriptor trainer saved at {self.descriptor_trainer_path}...')
